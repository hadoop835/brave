...
public class Tracing extends org.apache.cassandra.tracing.Tracing {
  ...
  @Override public final TraceState begin(String request, InetAddress client,
    ...
    span.name(spanName(state, request)).kind(Span.Kind.SERVER);
    requestTags(state, request, parameters, span);
  ...
  protected String spanName(TraceState state, String request) {
    return state.traceType.name();
  }

  /**
   * Adds any tags based on the incoming request from the client.
   *
   * <p>By default, this adds the {@link CassandraTraceKeys#CASSANDRA_REQUEST} and the {@link
   * CassandraTraceKeys#CASSANDRA_SESSION_ID}.
   */
  protected void requestTags(TraceState state, String request, Map<String, String> parameters,
      Tagger tagger) {
    tagger.tag(CassandraTraceKeys.CASSANDRA_REQUEST, request);
    tagger.tag(CassandraTraceKeys.CASSANDRA_SESSION_ID, state.sessionId.toString());
  }
